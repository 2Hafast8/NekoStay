...
export const bookingFormSchema = z
  .object({
    ...
    // Data Pemesanan
    class: z.string().trim().min(1, "Kelas kamar wajib dipilih"),
...
export const editBookingSchema = z.object({
  className: z.string().min(1, "Kelas kamar wajib dipilih"),
...
/**
 * Validasi Catatan Internal Admin (Multi-Admin Notes & Sticky Alerts)
 */
export const adminBookingNoteSchema = z.object({
  booking_id: z.string().uuid("ID booking harus berupa UUID valid").optional(),
  category: z.enum(
    ["urgent", "medical", "diet", "behavior", "shift_handoff", "general"],
    { message: "Kategori catatan tidak valid" }
  ),
  content: z
    .string()
    .trim()
    .min(1, "Konten catatan tidak boleh kosong")
    .max(1000, "Konten catatan maksimal 1000 karakter"),
  is_pinned: z.boolean().default(false),
});

/**
 * Validasi Perubahan Status Pembayaran Darurat / Manual oleh Admin
 */
export const emergencyPaymentStatusSchema = z.object({
  paymentStatus: z.enum(["Unpaid", "Paid", "Failed", "Refunded"], {
    message: "Status pembayaran tidak valid. Pilihan: Unpaid, Paid, Failed, Refunded",
  }),
  reason: z
    .string()
    .trim()
    .min(5, "Alasan darurat wajib diisi minimal 5 karakter sebagai catatan audit")
    .max(500, "Alasan darurat maksimal 500 karakter"),
});
