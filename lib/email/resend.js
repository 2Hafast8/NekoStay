...
/**
 * Safe email sending wrapper supporting 2 modes:
 * Mode 1 (Testing / Single Email): If using onboarding@resend.dev and RESEND_TEST_RECIPIENT is set,
 *          redirects outgoing emails to the owner's test email so emails can be tested without a custom domain.
 * Mode 2 (Production / Custom Domain): If RESEND_FROM_EMAIL is set to a verified custom domain,
 *          sends emails directly to each recipient's actual email address.
 */
async function safeSendEmail(emailPayload) {
  try {
    ...
    // Mode 1: Redirect to owner's test email if in testing mode
    ...
    if (res?.error) {
      ...
      if (res.error.message?.includes("testing emails")) {
        console.warn(
          `[Resend Notice] Mode Uji Coba: Resend membatasi pengiriman hanya ke email pemilik akun (${targetTo}). Tambahkan variabel RESEND_TEST_RECIPIENT=emailanda@gmail.com di Vercel agar email pengujian dialihkan ke inbox Anda.`
        );
      }
