...
export const DEFAULT_REFUND_PERCENTAGE = 90;
...
export function calculateRefund(
  ...
  const pct = (typeof refundPercentage === "number" && !isNaN(refundPercentage)) ? refundPercentage : DEFAULT_REFUND_PERCENTAGE;
  return Math.floor(remainingDays * pricePerDay * (pct / 100));
}
...
/**
 * Output kalkulasi untuk UI preview
 */
 ...
 */
export function getCheckoutCalculation(booking, actualCheckoutDate, refundPercentage = DEFAULT_REFUND_PERCENTAGE) {
  ...
  const actualCheckoutDayOnly = new Date(
    actualCheckoutDateObj.getFullYear(),
    actualCheckoutDateObj.getMonth(),
    actualCheckoutDateObj.getDate(),
  );
  ...
  const pct = (typeof refundPercentage === "number" && !isNaN(refundPercentage)) ? refundPercentage : 90;
  ...
  if (diffDays > 0) {
    ...
    refund = Math.floor(refundDays * booking.price_per_day * (pct / 100));
    notes = `Dijemput ${refundDays} hari lebih awal. Mendapatkan refund ${pct}% tarif per hari sisa.`;
  }

  const discount = Number(booking.discount_amount) || 0;
  const finalCost = Math.max(0, (Number(booking.estimated_total) || 0) - discount + lateFee - refund);
  ...
  return {
    ...
    refundAmount: refund,
    daysLate: lateDaysCount,
    earlyDays: refundDays,
  };
}
