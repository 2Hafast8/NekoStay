...
|---|---|---|
| **Framework** | Next.js 16.2.6 (App Router + Turbopack) | Server Components, REST API Handlers, Async Params |
| **Frontend Core** | React 19.2.4 | Modern React Actions, Hooks & Concurrent Features |
| **Database & Auth** | Supabase (PostgreSQL 15+) | Cloud DB + Auth SSR + Storage + Realtime WebSocket |
| **Styling & Icons** | Tailwind CSS v4 + Lucide Icons | Utility-first CSS + Modern icon set |
| **UI & Animations** | shadcn/ui + GSAP + Anime.js + Lenis | Accessible UI + Magnetic & Staggered Animations |
| **Payment Gateway** | Midtrans (Snap) + QR Scanner Kasir | Online Snap payment & Offline Cash QR verification |
