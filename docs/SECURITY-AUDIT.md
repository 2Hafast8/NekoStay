...
| **A04:2021** | **Insecure Design** | ✅ Aman | - Race-condition prevention saat pembatalan pesanan menggunakan lock atomik (`.eq('status', 'Menunggu')`).<br>- QR token pembayaran offline berbatas waktu 24 jam dan bersifat *one-time use* (`offline_token_used = true`).<br>- **In-Memory Sliding-Window Rate Limiter**: Endpoint verifikasi kupon dan referral dibatasi 30 request/menit per IP dengan auto-pruning memori (`lib/utils/rate-limit.js`).<br>- **Emergency Payment Status Audit Trail**: Perubahan status pembayaran darurat wajib memuat alasan audit formal (min 5 karakter) & konfirmasi tanggung jawab eksplisit via [`EmergencyPaymentModal`](../components/admin/EmergencyPaymentModal.jsx).<br>- **User-Centric Error Sanitizer**: Mencegah kebocoran skema DB, foreign key, atau token melalui kamus kesalahan ramah pengguna [`lib/utils/errors.js`](../lib/utils/errors.js). |
| **A05:2021** | **Security Misconfiguration** | ✅ Aman | - Header keamanan modern di `next.config.mjs`: `Content-Security-Policy: frame-ancestors 'none'`, `X-Frame-Options: DENY`, `X-Content-Type-Options: nosniff`, `Referrer-Policy: strict-origin-when-cross-origin`, `X-DNS-Prefetch-Control: on`, `Permissions-Policy`.<br>- Proteksi endpoint cron dengan bearer secret token.<br>- **Dual-Mode Error Architecture**: Accordion diagnostik teknis hanya aktif pada mode dev (`NODE_ENV !== "production"`). Pada mode produksi/deploy, pesan mentah, nama kolom SQL (`428C9`), dan petunjuk konfigurasi **dihilangkan 100% dari DOM & respons API** via [`sanitizeApiError`](../lib/utils/errors.js).<br>- **Proactive Turnstile Check**: Validasi awal token captcha pada form auth sebelum pengiriman request mencegah eksposisi pesan mentah Supabase. |
...
| **Anti-Bot Captcha** | Cloudflare Turnstile token validation proaktif | [`components/shared/SupabaseCaptcha.jsx`](../components/shared/SupabaseCaptcha.jsx) |
| **Error Sanitizer & Redaction** | Zero-leakage error format & Dev/Deploy dual mode | [`lib/utils/errors.js`](../lib/utils/errors.js), [`UserErrorAlert.jsx`](../components/shared/UserErrorAlert.jsx) |
| **Emergency Payment Audit** | Validasi alasan minimal 5 karakter & checkbox tanggung jawab | [`components/admin/EmergencyPaymentModal.jsx`](../components/admin/EmergencyPaymentModal.jsx) |
